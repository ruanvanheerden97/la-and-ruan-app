...
if menu == "🏠 Home":
    ...
    # show your shared image banner
    if os.path.exists("La_and_Ruan.jpg"):
        st.image("La_and_Ruan.jpg", use_container_width=True)
    st.markdown("---")
    ...
    for e in events:
        ...
        if st.session_state.get('edit_cal') == ridx:
            ...
            if st.button("Save Changes", key=f"save_cal_{ridx}"):
                calendar_ws.update(ridx, 1, [[
                    d.strftime("%Y-%m-%d"), t, de, p
                ]])
    ...
    with st.form("calendar_form"):
        ...
        if st.form_submit_button("Add") and t:
            calendar_ws.append_row([
                str(d), t, de, p, now.strftime("%Y-%m-%d %H:%M:%S"), "", ""
            ])
            notes, bucket_items, calendar_items, mood_entries, users = fetch_data()
            upcoming_events, past_events = get_events(calendar_items)
            st.success("Added!")
